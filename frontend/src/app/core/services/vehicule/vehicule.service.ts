...
import { VehiculelistModel, ApiResponse, ApiError } from 'src/app/store/Vehicule/vehicule.model';
...
export class VehiculeService {
  ...
  /**
   * Récupère les vehicule avec pagination
   */
  getAllVehicules(page: number = 1): Observable<ApiResponse<VehiculelistModel>> {
    ...
    return this.http.get<ApiResponse<VehiculelistModel>>(`${this.apiUrl}/vehicules/?page=${page}`, options)
      .pipe(
        tap(response => {
  ...
  /**
     * Récuperer les vehicules sans pagination
     */
  
    getListVehicules(): Observable<VehiculelistModel[]> {
      //const options = { headers: this.getAuthHeaders() };
    
      return this.http.get<VehiculelistModel[]>(`${this.apiUrl}/allnopagin/vehicules/`)
        .pipe(
          tap(response => {
  ...
  getVehiculeById(id: string): Observable<VehiculelistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.get<VehiculelistModel>(`${this.apiUrl}/vehicules/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
   ...
   */
  createVehicule(vehicule: Partial<VehiculelistModel>): Observable<VehiculelistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.post<VehiculelistModel>(`${this.apiUrl}/vehicules/`, vehicule, options)
      .pipe(
        catchError(this.handleError)
      );
  }
   ...
   */
  updateVehicule(vehicule: VehiculelistModel): Observable<VehiculelistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.put<VehiculelistModel>(`${this.apiUrl}/vehicules/${vehicule.id}/`, vehicule, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteVehicule(id: string): Observable<void> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.delete<void>(`${this.apiUrl}/vehicules/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteMultipleVehicule(ids: number[]): Observable<any> {
    ...
    return this.http.delete<any>(`${this.apiUrl}/vehicules/bulk_delete/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
