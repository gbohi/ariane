...
import { EtatlistModel, ApiResponse, ApiError } from 'src/app/store/Etat/etat.model';
...
export class EtatService {
  ...
  /**
   * Récupère les etat avec pagination
   */
  getAllEtats(page: number = 1): Observable<ApiResponse<EtatlistModel>> {
    ...
    return this.http.get<ApiResponse<EtatlistModel>>(`${this.apiUrl}/etats/?page=${page}`, options)
      .pipe(
        tap(response => {
  ...
  /**
   * Récupère un etat par son ID
   */
  getEtatById(id: string): Observable<EtatlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.get<EtatlistModel>(`${this.apiUrl}/etats/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  /**
   * Crée un nouveau etat
   */
  createEtat(etat: Partial<EtatlistModel>): Observable<EtatlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.post<EtatlistModel>(`${this.apiUrl}/etats/`, etat, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  updateEtat(etat: EtatlistModel): Observable<EtatlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.put<EtatlistModel>(`${this.apiUrl}/etats/${etat.id}/`, etat, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteEtat(id: string): Observable<void> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.delete<void>(`${this.apiUrl}/etats/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteMultipleEtat(ids: number[]): Observable<any> {
    ...
    console.log(ids);
    return this.http.delete<any>(`${this.apiUrl}/etats/bulk_delete/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
