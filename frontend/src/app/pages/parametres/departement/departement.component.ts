...
import { adddepartementData, deletedepartementData, deletemultipledepartementData, fetchdepartementData, updatedepartementData } from 'src/app/store/Departement/departement.action';
import { selectdepartementData, selectTotalItems, selectLoading, selectCurrentPage } from 'src/app/store/Departement/departement-selector';
...
export class DepartementComponent {
  ...
  @ViewChild('addDepartement', { static: false }) addDepartement?: ModalDirective;
  @ViewChild('deleteRecordModal', { static: false }) deleteRecordModal?: ModalDirective;
  @ViewChild('mainContainer', { static: false }) mainContainer?: ElementRef;
  @ViewChild('viewDepartement', { static: false }) viewDepartement?: ModalDirective;
  ...
  ngOnInit(): void {
    ...
    this.breadCrumbItems = [
      ...
      { label: 'Departement List', active: true }
    ];
    ...
    this.store.select(selectdepartementData).subscribe(data => {
      if (data) {
        ...
        this.departementList = [...this.departements];
  ...
  ngAfterViewInit() {
    ...
    if (this.addDepartement) {
      this.addDepartement.onHidden.subscribe(() => {
        ...
        if (modaltitle) modaltitle.innerHTML = 'Ajouter un departement';
  ...
  loadData(page: number = 1): void {
    this.store.dispatch(fetchdepartementData({ page }));
  }
  ...
  onUploadSuccess(event: any) {
    setTimeout(() => {
      ...
      this.departementForm.controls['img'].setValue(event[0].dataURL);
    }, 0);
  }
  ...
  openAddModal() {
    ...
    this.addDepartement?.show();
    const modaltitle = document.querySelector('.modal-title') as HTMLAreaElement;
    modaltitle.innerHTML = 'Ajouter un departement';
    const modalbtn = document.getElementById('add-btn') as HTMLAreaElement;
  ...
  editList(id: any) {
    ...
    this.addDepartement?.show();
    const modaltitle = document.querySelector('.modal-title') as HTMLAreaElement;
    modaltitle.innerHTML = 'Modifier le departement';
    const modalbtn = document.getElementById('add-btn') as HTMLAreaElement;
    modalbtn.innerHTML = 'Mettre à jour';
    
    // Trouver l'élément par index ou id
    const editData = typeof id === 'number' ? this.departements[id] : this.departements.find(p => p.id === id);
    ...
    if (editData) {
      ...
      this.departementForm.patchValue(editData);
    }
  }
  ...
  saveProperty() {
    if (this.departementForm.valid) {
      let formValue = { ...this.departementForm.value };
      ...
      if (this.departementForm.get('id')?.value) {
        // L'ID existe, donc on met à jour
        const updatedData = formValue;
        this.store.dispatch(updatedepartementData({ updatedData }));
        this.toastService.success('Departement mis à jour avec succès !', 'Succès');
      } else {
        // L'ID est vide, on crée une nouvelle entrée
        const newData = formValue;
        this.store.dispatch(adddepartementData({ newData }));
        this.toastService.success('Departement ajouté avec succès !', 'Succès');
      }
...
confirmDelete(id?: any) {
  if (id) {
    ...
    this.store.dispatch(deletedepartementData({ id: id.toString() }));
    this.toastService.success('Departement supprimé avec succès !', 'Succès');
    // Mettre à jour l'UI
  ...
  if (this.checkedValGet && this.checkedValGet.length > 0) {
    ...
    this.store.dispatch(deletemultipledepartementData({ id: this.checkedValGet.join(',') }));
    this.toastService.success('Departement supprimées avec succès !', 'Succès');
  ...
  this.toastService.error('Aucune departement sélectionné pour suppression.', 'Erreur');
...
private afterDeleteActions() {
  ...
  checkUncheckAll(ev: any) {
    // S'assurer que departement est défini
    if (!this.departements) return;
    
    // Mettre à jour l'état de tous les éléments
    this.departements.forEach((x: any) => {
      if (x) {
  ...
  private updateCheckedValues() {
    ...
    if (this.departements && this.departements.length > 0) {
      for (let i = 0; i < this.departements.length; i++) {
        if (this.departements[i] && this.departements[i].state === true) {
          checkedVal.push(this.departements[i].id);
        }
      }
    }
  ...
  onSort(column: any) {
    ...
    const sortedArray = [...this.departements]; // Create a new array
    sortedArray.sort((a, b) => {
      const res = this.compare(a[column], b[column]);
      return this.direction === 'asc' ? res : -res;
    });
    this.departements = sortedArray;
  }
  ...
  filterdata() {
    if (this.term) {
      this.departements = this.departementList.filter((el: any) => 
        el.libelle_service?.toLowerCase().includes(this.term.toLowerCase()) || 
  ...
  updateNoResultDisplay() {
    ...
    if (this.term && this.departements.length === 0) {
...
viewDetails(id: any) {
  ...
  const departementData = this.departements.find(p => p.id === id);
  
  if (departementData) {
    this.selectedDepartement = departementData;
    this.viewDepartement?.show();
  } else {
