...
import { addstatutData, deletestatutData, deletemultiplestatutData, fetchstatutData, updatestatutData } from 'src/app/store/Statut/statut.action';
import { selectstatutData, selectTotalItems, selectLoading, selectCurrentPage } from 'src/app/store/Statut/statut-selector';
...
export class StatutComponent {
  ...
  @ViewChild('addStatut', { static: false }) addStatut?: ModalDirective;
  @ViewChild('deleteRecordModal', { static: false }) deleteRecordModal?: ModalDirective;
  @ViewChild('mainContainer', { static: false }) mainContainer?: ElementRef;
  @ViewChild('viewStatut', { static: false }) viewStatut?: ModalDirective;
  ...
  ngOnInit(): void {
    ...
    this.breadCrumbItems = [
      { label: 'Statuts', active: true },
      { label: 'Statut List', active: true }
    ];
    ...
    this.statutForm = this.formBuilder.group({
      id: [''],
      libelle_statut: ['', [Validators.required]]
    });
    ...
    this.store.select(selectstatutData).subscribe(data => {
      if (data) {
  ...
  ngAfterViewInit() {
    ...
    if (this.addStatut) {
      this.addStatut.onHidden.subscribe(() => {
        ...
        if (modaltitle) modaltitle.innerHTML = 'Ajouter un statut';
  ...
  loadData(page: number = 1): void {
    this.store.dispatch(fetchstatutData({ page }));
  }
  ...
  onUploadSuccess(event: any) {
    setTimeout(() => {
      ...
      this.statutForm.controls['img'].setValue(event[0].dataURL);
    }, 0);
  }
  ...
  openAddModal() {
    ...
    this.addStatut?.show();
    const modaltitle = document.querySelector('.modal-title') as HTMLAreaElement;
    modaltitle.innerHTML = 'Ajouter un statut';
    const modalbtn = document.getElementById('add-btn') as HTMLAreaElement;
  ...
  /*closeAddModal() {
    ...
    const modaltitle = document.querySelector('.modal-title') as HTMLAreaElement;
    if (modaltitle) modaltitle.innerHTML = 'Ajouter une statut';
  ...
  editList(id: any) {
    ...
    this.addStatut?.show();
    const modaltitle = document.querySelector('.modal-title') as HTMLAreaElement;
    modaltitle.innerHTML = 'Modifier le statut';
    const modalbtn = document.getElementById('add-btn') as HTMLAreaElement;
    modalbtn.innerHTML = 'Mettre à jour';
    
    // Trouver l'élément par index ou id
    const editData = typeof id === 'number' ? this.statuts[id] : this.statuts.find(p => p.id === id);
    ...
    if (editData) {
      ...
      this.statutForm.patchValue(editData);
    }
  }
  ...
  saveProperty() {
    if (this.statutForm.valid) {
      let formValue = { ...this.statutForm.value };
      ...
      if (this.statutForm.get('id')?.value) {
        // L'ID existe, donc on met à jour
        const updatedData = formValue;
        this.store.dispatch(updatestatutData({ updatedData }));
        this.toastService.success('Statut mis à jour avec succès !', 'Succès');
      } else {
        // L'ID est vide, on crée une nouvelle entrée
        const newData = formValue;
        this.store.dispatch(addstatutData({ newData }));
        this.toastService.success('Statut ajouté avec succès !', 'Succès');
      }
...
confirmDelete(id?: any) {
  if (id) {
    ...
    this.store.dispatch(deletestatutData({ id: id.toString() }));
    this.toastService.success('Statut supprimé avec succès !', 'Succès');
    // Mettre à jour l'UI
  ...
  if (this.checkedValGet && this.checkedValGet.length > 0) {
    ...
    this.store.dispatch(deletemultiplestatutData({ id: this.checkedValGet.join(',') }));
    this.toastService.success('Statut supprimées avec succès !', 'Succès');
  ...
  this.toastService.error('Aucune statut sélectionné pour suppression.', 'Erreur');
...
private afterDeleteActions() {
  ...
  checkUncheckAll(ev: any) {
    // S'assurer que statut est défini
    if (!this.statuts) return;
    
    // Mettre à jour l'état de tous les éléments
    this.statuts.forEach((x: any) => {
      if (x) {
  ...
  private updateCheckedValues() {
    ...
    if (this.statuts && this.statuts.length > 0) {
      for (let i = 0; i < this.statuts.length; i++) {
        if (this.statuts[i] && this.statuts[i].state === true) {
          checkedVal.push(this.statuts[i].id);
        }
      }
    }
  ...
  onSort(column: any) {
    ...
    const sortedArray = [...this.statuts]; // Create a new array
    sortedArray.sort((a, b) => {
      const res = this.compare(a[column], b[column]);
      return this.direction === 'asc' ? res : -res;
    });
    this.statuts = sortedArray;
  }
  ...
  filterdata() {
    if (this.term) {
      this.statuts = this.statutList.filter((el: any) => 
        el.libelle_statut?.toLowerCase().includes(this.term.toLowerCase()) || 
  ...
  updateNoResultDisplay() {
    ...
    if (this.term && this.statuts.length === 0) {
...
viewDetails(id: any) {
  ...
  const statutData = this.statuts.find(p => p.id === id);
  
  if (statutData) {
    this.selectedStatut = statutData;
    this.viewStatut?.show();
  } else {
    this.toastService.error('Statut introuvable', 'Erreur');
  }
}
