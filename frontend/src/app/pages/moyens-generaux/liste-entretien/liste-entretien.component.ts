...
import { addentretienvehiculeData, deleteentretienvehiculeData, deletemultipleentretienvehiculeData, fetchentretienvehiculeData, updateentretienvehiculeData } from 'src/app/store/Entretienvehicule/entretienvehicule.action';
import { selectentretienvehiculeData, selectTotalItems, selectLoading, selectCurrentPage } from 'src/app/store/Entretienvehicule/entretienvehicule-selector';
import { PageChangedEvent } from 'ngx-bootstrap/pagination';
import { cloneDeep } from 'lodash';


@Component({
 selector: 'app-liste-entretien',
  templateUrl: './liste-entretien.component.html',
  styleUrl: './liste-entretien.component.scss',
  providers: [DecimalPipe]
})

// list Component
export class ListeEntretienComponent {

  // bread crumb items
  breadCrumbItems!: Array<{}>;

  entretienvehicules: any[] = [];
  entretienvehiculeList: any[] = [];
  totalItems: number = 0;
  currentPage: number = 1;
  isLoading: boolean = true;

  entretienvehiculeForm!: UntypedFormGroup;
  submitted = false;
  ...
  @ViewChild('addEntretienvehicule', { static: false }) addEntretienvehicule?: ModalDirective;
  @ViewChild('deleteRecordModal', { static: false }) deleteRecordModal?: ModalDirective;
  @ViewChild('mainContainer', { static: false }) mainContainer?: ElementRef;
  @ViewChild('viewEntretienvehicule', { static: false }) viewEntretienvehicule?: ModalDirective;
  selectedEntretienvehicule: any = null;
  deleteID: any;
  direction: any = 'asc';
  ...
  ngOnInit(): void {
    ...
    this.breadCrumbItems = [
      { label: 'Entretien véhicules', active: true },
      { label: 'Entretien véhicules List', active: true }
    ];

    /**
     * Form Validation
     */
    this.entretienvehiculeForm = this.formBuilder.group({
      id: [''],
    ...
    this.store.select(selectentretienvehiculeData).subscribe(data => {
      if (data) {
        // Initialiser correctement les états
        this.entretienvehicules = data.map(item => ({
          ...item,
          state: false
        }));
        this.entretienvehiculeList = [...this.entretienvehicules];
  ...
  loadData(page: number = 1): void {
    this.store.dispatch(fetchentretienvehiculeData({ page }));
  }
  ...
  onUploadSuccess(event: any) {
    setTimeout(() => {
      ...
      this.entretienvehiculeForm.controls['img'].setValue(event[0].dataURL);
    }, 0);
  }
  ...
  openAddModal() {
    ...
    this.addEntretienvehicule?.show();
    const modaltitle = document.querySelector('.modal-title') as HTMLAreaElement;
    modaltitle.innerHTML = 'Ajouter un entretien vehicule';
    const modalbtn = document.getElementById('add-btn') as HTMLAreaElement;
  ...
  /*closeAddModal() {
    this.addEntretienvehicule?.hide();
  ...
  editList(id: any) {
    ...
    this.addEntretienvehicule?.show();
    const modaltitle = document.querySelector('.modal-title') as HTMLAreaElement;
    modaltitle.innerHTML = 'Modifier le entretien vehicule';
    const modalbtn = document.getElementById('add-btn') as HTMLAreaElement;
    modalbtn.innerHTML = 'Mettre à jour';
    
    // Trouver l'élément par index ou id
    const editData = typeof id === 'number' ? this.entretienvehicules[id] : this.entretienvehicules.find(p => p.id === id);
    ...
    if (editData) {
      ...
      this.entretienvehiculeForm.patchValue(editData);
    }
  }

  // Add/Update Property
  saveProperty() {
    if (this.entretienvehiculeForm.valid) {
      let formValue = { ...this.entretienvehiculeForm.value };
      ...
      if (this.entretienvehiculeForm.get('id')?.value) {
        // L'ID existe, donc on met à jour
        const updatedData = formValue;
        this.store.dispatch(updateentretienvehiculeData({ updatedData }));
        this.toastService.success('Typebeoin mis à jour avec succès !', 'Succès');
      } else {
        // L'ID est vide, on crée une nouvelle entrée
        const newData = formValue;
        this.store.dispatch(addentretienvehiculeData({ newData }));
        this.toastService.success('Entretien vehicule ajouté avec succès !', 'Succès');
      }
...
confirmDelete(id?: any) {
  if (id) {
    ...
    this.store.dispatch(deleteentretienvehiculeData({ id: id.toString() }));
    this.toastService.success('ENtretien vehicule supprimé avec succès !', 'Succès');
    // Mettre à jour l'UI
  ...
  if (this.checkedValGet && this.checkedValGet.length > 0) {
    ...
    this.store.dispatch(deletemultipleentretienvehiculeData({ id: this.checkedValGet.join(',') }));
    this.toastService.success('Entretien vehicule supprimées avec succès !', 'Succès');
  ...
  this.toastService.error('Aucun entretien vehicule sélectionné pour suppression.', 'Erreur');
...
private afterDeleteActions() {
  ...
  resetCheckboxes() {
    if (this.entretienvehicules) {
      this.entretienvehicules.forEach((item: any) => {
  ...
  checkUncheckAll(ev: any) {
    // S'assurer que entretien vehicule est défini
    if (!this.entretienvehicules) return;
    
    // Mettre à jour l'état de tous les éléments
    this.entretienvehicules.forEach((x: any) => {
      if (x) {
  ...
  private updateCheckedValues() {
    ...
    if (this.entretienvehicules && this.entretienvehicules.length > 0) {
      for (let i = 0; i < this.entretienvehicules.length; i++) {
        if (this.entretienvehicules[i] && this.entretienvehicules[i].state === true) {
          checkedVal.push(this.entretienvehicules[i].id);
        }
      }
    }
  ...
  onSort(column: any) {
    ...
    const sortedArray = [...this.entretienvehicules]; // Create a new array
    sortedArray.sort((a, b) => {
      const res = this.compare(a[column], b[column]);
      return this.direction === 'asc' ? res : -res;
    });
    this.entretienvehicules = sortedArray;
  }
  ...
  filterdata() {
    if (this.term) {
      this.entretienvehicules = this.entretienvehiculeList.filter((el: any) => 
  ...
  updateNoResultDisplay() {
    ...
    if (this.term && this.entretienvehicules.length === 0) {
...
viewDetails(id: any) {
  ...
  const entretienvehiculeData = this.entretienvehicules.find(p => p.id === id);
  
  if (entretienvehiculeData) {
    this.selectedEntretienvehicule = entretienvehiculeData;
    this.viewEntretienvehicule?.show();
  } else {
...
closeViewModal() {
  ...
  setTimeout(() => {
    this.viewEntretienvehicule?.hide();
    this.selectedEntretienvehicule = null;
  }, 10);
}
