...
import { addetatData, deleteetatData, deletemultipleetatData, fetchetatData, updateetatData } from 'src/app/store/Etat/etat.action';
import { selectetatData, selectTotalItems, selectLoading, selectCurrentPage } from 'src/app/store/Etat/etat-selector';
...
export class EtatComponent {
  ...
  ngAfterViewInit() {
    ...
    if (this.addEtat) {
      this.addEtat.onHidden.subscribe(() => {
        ...
        if (modaltitle) modaltitle.innerHTML = 'Ajouter une état';
  ...
  onUploadSuccess(event: any) {
    setTimeout(() => {
      ...
      this.etatForm.controls['img'].setValue(event[0].dataURL);
    }, 0);
  }
  ...
  openAddModal() {
    ...
    this.addEtat?.show();
    const modaltitle = document.querySelector('.modal-title') as HTMLAreaElement;
    modaltitle.innerHTML = 'Ajouter une état';
    const modalbtn = document.getElementById('add-btn') as HTMLAreaElement;
  ...
  editList(id: any) {
    ...
    this.addEtat?.show();
    const modaltitle = document.querySelector('.modal-title') as HTMLAreaElement;
    modaltitle.innerHTML = 'Modifier la état';
    const modalbtn = document.getElementById('add-btn') as HTMLAreaElement;
    modalbtn.innerHTML = 'Mettre à jour';
    
    // Trouver l'élément par index ou id
    const editData = typeof id === 'number' ? this.etats[id] : this.etats.find(p => p.id === id);
  ...
  saveProperty() {
    if (this.etatForm.valid) {
      let formValue = { ...this.etatForm.value };
      ...
      if (this.etatForm.get('id')?.value) {
        // L'ID existe, donc on met à jour
        const updatedData = formValue;
        this.store.dispatch(updateetatData({ updatedData }));
        this.toastService.success('état mise à jour avec succès !', 'Succès');
      } else {
        // L'ID est vide, on crée une nouvelle entrée
        const newData = formValue;
        this.store.dispatch(addetatData({ newData }));
        this.toastService.success('état ajoutée avec succès !', 'Succès');
      }
...
confirmDelete(id?: any) {
  if (id) {
    ...
    this.toastService.success('état supprimée avec succès !', 'Succès');
    // Mettre à jour l'UI
  ...
  if (this.checkedValGet && this.checkedValGet.length > 0) {
    ...
    this.toastService.success('états supprimées avec succès !', 'Succès');
  ...
  this.toastService.error('Aucune état sélectionnée pour suppression.', 'Erreur');
...
private afterDeleteActions() {
  ...
  checkUncheckAll(ev: any) {
    // S'assurer que etats est défini
    if (!this.etats) return;
    
    // Mettre à jour l'état de tous les éléments
    this.etats.forEach((x: any) => {
      if (x) {
  ...
  private updateCheckedValues() {
    ...
    if (this.etats && this.etats.length > 0) {
      for (let i = 0; i < this.etats.length; i++) {
        if (this.etats[i] && this.etats[i].state === true) {
          checkedVal.push(this.etats[i].id);
        }
      }
    }
  ...
  onSort(column: any) {
    ...
    const sortedArray = [...this.etats]; // Create a new array
    sortedArray.sort((a, b) => {
      const res = this.compare(a[column], b[column]);
      return this.direction === 'asc' ? res : -res;
    });
    this.etats = sortedArray;
  }
  ...
  filterdata() {
    if (this.term) {
      this.etats = this.etatList.filter((el: any) => 
  ...
  updateNoResultDisplay() {
    ...
    if (this.term && this.etats.length === 0) {
