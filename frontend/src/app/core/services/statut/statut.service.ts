...
import { StatutlistModel, ApiResponse, ApiError } from 'src/app/store/Statut/statut.model';
...
export class StatutService {
  ...
  /**
   * Récupère les statuts avec pagination
   */
  getAllStatuts(page: number = 1): Observable<ApiResponse<StatutlistModel>> {
    ...
    return this.http.get<ApiResponse<StatutlistModel>>(`${this.apiUrl}/statuts/?page=${page}`, options)
      .pipe(
        tap(response => {
  ...
  /**
       * Récuperer les statut sans pagination
       */
    
      getListStatut(): Observable<StatutlistModel[]> {
        //const options = { headers: this.getAuthHeaders() };
      
        return this.http.get<StatutlistModel[]>(`${this.apiUrl}/allnopagin/statuts/`)
          .pipe(
            tap(response => {
  ...
  getStatutById(id: string): Observable<StatutlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.get<StatutlistModel>(`${this.apiUrl}/statuts/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
   ...
   */
  createStatut(statut: Partial<StatutlistModel>): Observable<StatutlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.post<StatutlistModel>(`${this.apiUrl}/statuts/`, statut, options)
      .pipe(
        catchError(this.handleError)
      );
  }
   ...
   */
  updateStatut(statut: StatutlistModel): Observable<StatutlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.put<StatutlistModel>(`${this.apiUrl}/statuts/${statut.id}/`, statut, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteStatut(id: string): Observable<void> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.delete<void>(`${this.apiUrl}/statuts/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteMultipleStatut(ids: number[]): Observable<any> {
    ...
    return this.http.delete<any>(`${this.apiUrl}/statuts/bulk_delete/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
