...
import { TypebesoinlistModel, ApiResponse, ApiError } from 'src/app/store/Typebesoin/typebesoin.model';
...
export class TypebesoinService {
  ...
  /**
   * Récupère les typebesoin avec pagination
   */
  getAllTypebesoins(page: number = 1): Observable<ApiResponse<TypebesoinlistModel>> {
    ...
    return this.http.get<ApiResponse<TypebesoinlistModel>>(`${this.apiUrl}/typebesoins/?page=${page}`, options)
      .pipe(
        tap(response => {
  ...
  /**
     * Récuperer les Typebesoins sans pagination
     */
  
    getListTypebesoins(): Observable<TypebesoinlistModel[]> {
      //const options = { headers: this.getAuthHeaders() };
    
      return this.http.get<TypebesoinlistModel[]>(`${this.apiUrl}/public/typebesoins/`)
        .pipe(
          tap(response => {
  ...
  /**
   * Récupère un typebesoin par son ID
   */
  getTypebesoinById(id: string): Observable<TypebesoinlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.get<TypebesoinlistModel>(`${this.apiUrl}/typebesoins/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  /**
   * Crée un nouveau typebesoin
   */
  createTypebesoin(typebesoin: Partial<TypebesoinlistModel>): Observable<TypebesoinlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.post<TypebesoinlistModel>(`${this.apiUrl}/typebesoins/`, typebesoin, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  /**
   * Met à jour un typebesoin existant
   */
  updateTypebesoin(typebesoin: TypebesoinlistModel): Observable<TypebesoinlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.put<TypebesoinlistModel>(`${this.apiUrl}/typebesoins/${typebesoin.id}/`, typebesoin, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  /**
   * Supprime un typebesoin
   */
  deleteTypebesoin(id: string): Observable<void> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.delete<void>(`${this.apiUrl}/typebesoins/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteMultipleTypebesoin(ids: number[]): Observable<any> {
    ...
    return this.http.delete<any>(`${this.apiUrl}/typebesoins/bulk_delete/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
