...
import { AgencelistModel, ApiResponse, ApiError } from 'src/app/store/Agence/agence.model';
...
export class AgenceService {
  ...
  /**
   * Récupère les agence avec pagination
   */
  getAllAgences(page: number = 1): Observable<ApiResponse<AgencelistModel>> {
    ...
    return this.http.get<ApiResponse<AgencelistModel>>(`${this.apiUrl}/agences/?page=${page}`, options)
      .pipe(
        tap(response => {
  ...
  /**
       * Récuperer les Agences sans pagination
       */
  ...
  /**
   * Récupère un agence par son ID
   */
  getAgenceById(id: string): Observable<AgencelistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.get<AgencelistModel>(`${this.apiUrl}/agences/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  /**
   * Crée un nouveau agence
   */
  createAgence(agence: Partial<AgencelistModel>): Observable<AgencelistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.post<AgencelistModel>(`${this.apiUrl}/agences/`, agence, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  /**
   * Met à jour un agence existant
   */
  updateAgence(agence: AgencelistModel): Observable<AgencelistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.put<AgencelistModel>(`${this.apiUrl}/agences/${agence.id}/`, agence, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteAgence(id: string): Observable<void> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.delete<void>(`${this.apiUrl}/agences/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteMultipleAgence(ids: number[]): Observable<any> {
    ...
    return this.http.delete<any>(`${this.apiUrl}/agences/bulk_delete/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
