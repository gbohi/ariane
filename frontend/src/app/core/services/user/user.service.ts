...
import { UserlistModel, ApiResponse, ApiError } from 'src/app/store/User/user.model';
...
export class UserService {
  ...
  /**
   * Récupère les user avec pagination
   */
  getAllUsers(page: number = 1): Observable<ApiResponse<UserlistModel>> {
    const options = { headers: this.getAuthHeaders() };
    this.currentPage = page; // Mise à jour de la page courante
    
    return this.http.get<ApiResponse<UserlistModel>>(`${this.apiUrl}/users/?page=${page}`, options)
      .pipe(
        tap(response => {
   ...
   */
  getUserById(id: string): Observable<UserlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.get<UserlistModel>(`${this.apiUrl}/users/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
   ...
   */
  createUser(user: Partial<UserlistModel>): Observable<UserlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.post<UserlistModel>(`${this.apiUrl}/users/`, user, options)
      .pipe(
        catchError(this.handleError)
      );
  }
   ...
   */
  updateUser(user: UserlistModel): Observable<UserlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.put<UserlistModel>(`${this.apiUrl}/users/${user.id}/`, user, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteMultipleUser(ids: number[]): Observable<any> {
    ...
    return this.http.delete<any>(`${this.apiUrl}/users/bulk_delete/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
