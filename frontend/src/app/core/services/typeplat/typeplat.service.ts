...
import { TypeplatlistModel, ApiResponse, ApiError } from 'src/app/store/Typeplat/typeplat.model';
...
export class TypeplatService {
  ...
  getAllTypeplats(page: number = 1): Observable<ApiResponse<TypeplatlistModel>> {
    ...
    return this.http.get<ApiResponse<TypeplatlistModel>>(`${this.apiUrl}/typeplats/?page=${page}`, options)
      .pipe(
        tap(response => {
  ...
  /**
     * Récuperer les Typeplats sans pagination
     */
  
    getListTypeplats(): Observable<TypeplatlistModel[]> {
      //const options = { headers: this.getAuthHeaders() };
    
      return this.http.get<TypeplatlistModel[]>(`${this.apiUrl}/allnopagin/typeplats/`)
        .pipe(
          tap(response => {
  ...
  getTypeplatById(id: string): Observable<TypeplatlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.get<TypeplatlistModel>(`${this.apiUrl}/typeplats/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
   ...
   */
  createTypeplat(typeplat: Partial<TypeplatlistModel>): Observable<TypeplatlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.post<TypeplatlistModel>(`${this.apiUrl}/typeplats/`, typeplat, options)
      .pipe(
        catchError(this.handleError)
      );
  }
   ...
   */
  updateTypeplat(typeplat: TypeplatlistModel): Observable<TypeplatlistModel> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.put<TypeplatlistModel>(`${this.apiUrl}/typeplats/${typeplat.id}/`, typeplat, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteTypeplat(id: string): Observable<void> {
    const options = { headers: this.getAuthHeaders() };
    return this.http.delete<void>(`${this.apiUrl}/typeplats/${id}/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteMultipleTypeplat(ids: number[]): Observable<any> {
    ...
    return this.http.delete<any>(`${this.apiUrl}/typeplats/bulk_delete/`, options)
      .pipe(
        catchError(this.handleError)
      );
  }
